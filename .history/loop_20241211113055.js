...
function longestSurname(students) {
  let longest = students.map((student) => {
    student.forEach((_alumn) => {
      if (_alumn.surname.length > longest.surname.length) {
        longest = student
      }
    })
  })
...
console.log("Functions: ", loopStudents(students), youngestStudent(students), longestSurname(students))